...
public class PayCollectSuccessController {
    ...
    public void setToCostumer() {
        JSONObject lastCollect = db.findLastCollectOfCostumer(costumerAccount.getPixKey());
        if(lastCollect.get("pixKey").toString().isEmpty()){
            this.toCostumer.setText(lastCollect.get("number").toString());
        }else{
            this.toCostumer.setText(lastCollect.get("pixKey").toString());
        }
    }
